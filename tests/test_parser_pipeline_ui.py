...
class ParserPipelineUiTest(unittest.TestCase):
    ...
    def test_auto_experiment_html_lists_each_case_cer_wer_readably(self):
        ...
        self.assertIn("Strict Method Summary", html)
        self.assertIn("Best CER", html)
        self.assertIn("Keyword + LLM", html)
        self.assertIn("Baseline", html)
        self.assertIn("10.0000%", html)
        self.assertIn("20.0000%", html)
        self.assertIn("+20.0000%", html)
        self.assertIn("ΔCER vs baseline", html)
        self.assertIn("Strict result", html)
        self.assertIn("Ties", html)
        self.assertIn("Latency", html)
        self.assertIn("asrpp-auto-case-tag", html)
        self.assertIn("Baseline (all toggles off)", html)
    ...
    def test_auto_experiment_html_adds_collapsible_character_diff_per_case(self):
        ...
        self.assertIn("/gradio_api/file=", html)
        self.assertIn("asrpp-auto-case-tag", html)
    ...
    def test_ui_accepts_large_audio_file_input(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("/gradio_api/file=", diff)
            self.assertIn("Audio upload cache", status)
            self.assertIn("available", gpu_status)
