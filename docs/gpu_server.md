...
- Noise reduction and volume normalization are separate experimental variables. WAV input is processed internally; compressed audio such as MP3 is converted through `ffmpeg` from `PATH` or the bundled `imageio-ffmpeg` binary.
