...
- Audio input, reference transcript input, preprocessing toggles, Keyword Bias, LLM post-processing, RAG, Search, model selection, weights, raw/corrected transcript viewer, diff, metrics, edits, and run status are wired through the Gradio UI.
...
- Sweep runs the A-G research conditions and writes `sweep_summary.csv` plus `sweep_analysis.json` with sweet-spot and over-bias detection.
...
- RNNoise and BS-RoFormer require configured external command templates before those preprocessing branches can be used in real experiments.
